...
META_SCHEMA = {"channel": str, "start": pl.Datetime, "end": pl.Datetime}
...
def create_transcription_embeddings(model, transcription_file, output_dir):
    ...
    for start_idx in range(0, trs_table.shape[0]):
        ...
        if len(full_text) > 0:
            logger.info(f"    ~ {start_idx} ::: {start_channel} {start_start} {current_end} - {current_end - start_start}")
            ft_emb = model.encode(full_text, show_progress_bar=False, normalize_embeddings=True)
            np.save(f, ft_emb)
            this = pl.DataFrame({"channel": start_channel, "start": start_start, "end": current_end})
...
def process_embeddings(model, press_embeddings, output_dir):
...
def main(args):
    ...
    process_embeddings(model, press_embeddings, args.output)
