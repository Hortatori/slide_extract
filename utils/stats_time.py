...
def stats(args) :
    ...
    df_to_save = pl.DataFrame(schema={"channel":str, "transcription_jour":pl.Float64, "extract_jour":pl.Float64,"percent":pl.Float64, "day":str})
    ls = []
    ...
    for jour, data in trs.group_by("jour"): 
        ...
        trs_duration = (
            ...
            .agg([
                pl.sum("duration").alias("transcription_jour")
            ])
        )
        ...
        extract_jour = (
            filter_jour.group_by("channel").agg([
                pl.sum("duration").alias("extract_jour")
            ])
        )   
    ...
    # no order but it s only for total on alll period
    total = pl.concat(ls)
    total_time = (
        total
        .group_by("channel")
